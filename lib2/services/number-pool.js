...
function fetchAvailableNumbers(smsApi) {
  return (0, _utils.promiseToFuture)(smsApi.getAvailableNumbers);
}
...
function createNumberPool(smsApi, cache) {
  var nums = _ramda2.default.ifElse(function (k) {
    ...
    return _ramdaFantasy.Future.of(cache.get(k));
  }, function (k) {
    return fetchAvailableNumbers(smsApi).map(cacheValue(k, cache));
  });

  return _ramdaFantasy.Future.of(NP_CACHE_KEY).chain(nums).map(createNodeRing);
}
