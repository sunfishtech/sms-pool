...
function Plivo() {
  ...
  return new _smsApi.SmsApi({
    ...
    getAvailableNumbers: function getAvailableNumbers() {
      ...
      return _rx.Observable.fromPromise(http.get(url, { auth: auth })).map(extractNums);
    },
    ...
    sendMessage: function sendMessage(message) {
      ...
      return _rx.Observable.fromPromise(http.post(url, messageToPayload(message), { auth: auth })).retry(3).map(extractId);
    }
  });
}
