...
function sendMessage(message, smsApi) {
  return (0, _utils.promiseToFuture)(smsApi.sendMessage, message).map(function (vendorId) {
  ...
  return (0, _ramdaFantasy.Reader)(function (env) {
    ...
    return {
      ...
      sendMessage: function sendMessage(message) {
        return throttledSendMessage(message, env.smsApi, limiter).map((0, _smsMessage.setStatus)(_smsMessage.STATUS.SENT));
      }
    };
  });
}
